...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_lataus_negatiivisella_summalla_ei_muuta_saldoa(self):
        kortti = Maksukortti(200)
        kortti.lataa_rahaa(-200)

        self.assertEqual(str(kortti), "Kortilla on rahaa 2.00 euroa")

    def test_kortilla_pystyty_ostamaan_edullisen_lounaan_kun_kortilla_sama_maara_kuin_hinta(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")
    
    def test_kortilla_pystyty_ostamaan_maukkaan_lounaan_kun_kortilla_sama_maara_kuin_hinta(self):
